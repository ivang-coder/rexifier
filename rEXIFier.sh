...
#   20201216 v1.1: changed README, function names and miscelanious
#   20210108 v1.2: added filename deduplication process for duplicates
#=======================================================================
...
if ( ! command -v exiftool &> /dev/null ) ; then
  printf "It appears 'exiftool' is not installed or it could not be found\n"
  exit
else
...
if [[ ${#} -lt 1 ]] ; then
  printf "At least one argument is expected, exiting\n"
  printf "${HelpTip}"
  exit
fi
...
case "${1}" in 
  ...
  /*|.) 
    ...
    if [[ ! -e ${1} ]] ; then
      printf "Source path ${1} could not be found or does not exist, exiting\n"
  ...
  *) 
    echo "Mandatory source path is invalid or could not be identified: ${1}, exiting\n"
...
case "${2}" in 
  ...
  /*|.) 
    ...
    if [[ ! -e ${2} ]] ; then
      printf "Destination path ${2} could not be found or does not exist, exiting\n"
  ...
  # Exiting if no expected parameter found
  *) 
    echo "Mandatory destination path is invalid or could not be identified: ${2}, exiting\n"
...
# Applying default parameters if the required arguments have not been passed
  ...
  printf "No optional parameters have been passed, applying defaults\n"
fi
...
for WIPSortedFileAbsolutePath in ${WIPSortedFileAbsolutePaths[@]}; do
  ...
  if [ -e ${WIPSortedFileAbsolutePath} ] && [ ${#WIPSortedFileAbsolutePath} != 0 ] ; then
    ...
    # Modify filename as per format if tag/attribute extraction is successful
      ...
      FormatedFileNameNoExtension=${MODEL}-${YEAR}${MONTH}${DAY}-${HOUR}${MINUTE}${SECOND}-${SUBSECOND}
      FormatedShortenedFileName=${MODEL}-${YEAR}${MONTH}${DAY}-${HOUR}${MINUTE}${SECOND}-
    fi
